let getProductsOfAllIntsExceptAtIndex = (intArray) => {
  ...
  if (intArray.length < 2) {
    throw new Error('array must contain at least 2 ints');
  }

  let prodsOfIntsExceptAtIndex = [];

  let prodSoFar = 1;
  for (let i = 0; i < intArray.length; i++) {
    prodsOfIntsExceptAtIndex[i] = prodSoFar;
    prodSoFar *= intArray[i];
  }

  prodSoFar = 1;
  for (let j = intArray.length - 1; j >= 0; j--) {
    prodsOfIntsExceptAtIndex[j] *= prodSoFar;
    prodSoFar *= intArray[j];
  }

  return prodsOfIntsExceptAtIndex;
};

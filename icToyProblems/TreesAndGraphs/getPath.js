...
function pathCreator(pathFinder, startNode, endNode) {
  let shortestPath = [];
  let currentNode = endNode;

  while (currentNode !== null) {
    shortestPath.push(currentNode);
    currentNode = pathFinder[currentNode];
  }
...
function getPath(graph, startNode, endNode) {
  ...
  if (!graph.hasOwnProperty(startNode) || !graph.hasOwnProperty(endNode)) {
    throw new Error('Graph does note contain the given start or end node');
  }
  ...
  while (nodesToVisit.size > 0) {
    ...
    if (currentNode === endNode) {
      return pathCreator(pathFinder, startNode, endNode);
    }

    graph[currentNode].forEach((neighbor) => {
      if (!pathFinder.hasOwnProperty(neighbor)) {
        nodesToVisit.enqueue(neighbor);
        pathFinder[neighbor] = currentNode;
      }
    });
  }

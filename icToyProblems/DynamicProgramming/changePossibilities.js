function changePossibilities(amount, denominations) {
  ...
  let centsPossArray = new Array(amount + 1).fill(0);
  centsPossArray[0] = 1;

  denominations.forEach((coin) => {
    for (let higherAmount = coin; higherAmount <= amount; higherAmount++) {
      let remainingAmount = higherAmount - coin;
      centsPossArray[higherAmount] += centsPossArray[remainingAmount];
    }
  });

  return centsPossArray[amount];
}

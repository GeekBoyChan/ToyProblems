function maxDuffelBagValue(cakeTypes, weightCapacity) {
  ...
  let capacityMaxValues = new Array(weightCapacity + 1).fill(0);
    ...
    for (let i = 0; i < cakeTypes.length; i++) {
      ...
      if (cakeType.weight <= currentCapacity) {
        currentMaxVal = Math.max(
          currentMaxVal,
          cakeType.value + capacityMaxValues[currentCapacity - cakeType.weight]
        );
      }
    }

    capacityMaxValues[currentCapacity] = currentMaxVal;
  }

  return capacityMaxValues[weightCapacity];
}
